...
class LossAccPlotter(object):
    ...
    def add_values(self, x_index, loss_train=None, loss_val=None, acc_train=None, acc_val=None, redraw=True):
    ...
    def redraw(self):
        ...
        ncol = 1
        labels = ["$CHART discriminator", "$CHART generator"]
        if self.show_averages:
            labels.extend(["$CHART discriminator (avg %d)" % (self.averages_period,),
                           "$CHART generator (avg %d)" % (self.averages_period,)])
            ncol += 1
        if self.show_regressions:
            labels.extend(["$CHART discriminator (regression)",
                           "$CHART generator (regression)"])
            ncol += 1
    ...
    def _redraw_main_lines(self):
        ...
        if ax1:
            h_lt, = ax1.plot(list(self.values_loss_train.keys()), list(self.values_loss_train.values()),
                             ls_loss_train, label="Generator", alpha=alpha_main)
            h_lv, = ax1.plot(list(self.values_loss_val.keys()), list(self.values_loss_val.values()),
                             ls_loss_val, label="Discriminator", alpha=alpha_main)
