...
class Generator:
    ...
    def __init__(self, experiment_name, object_class="", n_pc_points=2048, bneck_size=128, ae_loss='emd'):
        ...
        self.all_pc_data = load_all_point_clouds_under_folder(self.class_dir, n_threads=8, file_ending='.ply', verbose=True)
    ...
    def restore_gan(self, epoch = 1000):
        ...
        self.gan_model.restore_model("./models_checkpoints/", epoch, verbose=True)
    ...
    def interpolate(self, _from, _to, steps=10):
        ...
        for i, interpolation in enumerate(self.interpolations):
            obj_wrapper(interpolation, self.object_class + "_intr", i)
            #generate_mitsuba_xml(interpolation, self.object_class, i, variation=False)
            
        print("Interpolation Complete")
        print(self.interpolations.shape)
